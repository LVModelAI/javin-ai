...
export const getPolygonOnchainDataUsingZerion = tool({
  description: "Get real-time data from Polygon based blockchains.",
  parameters: z.object({
  ...
  execute: async ({ userQuery }: { userQuery?: string }) => {
    try {
      ...
      const aiAgentResponse = await generateText({
        ...
        system: `You are an intelligent API assistant. Your job is to process user queries and provide the most relevant polygon blockchain data in a user-friendly format.
        ...
        2. **Retrieve Required Parameters**:  
           ...
           - Pass the chain as polygon when neccesary.
      ...
      console.error("Error in getPolygonOnchainDataUsingZerion:", error);
