...
export const deepSearch = (dataStream: any) =>
  tool({
    description:
      "Perform a reasoned web and on-chain search with multiple steps and sources.",
    parameters: z.object({
      topic: z.string().describe("The main topic or question to answer"),
      depth: z
        .enum(["basic", "advanced"])
    ...
    execute: async ({
      ...
      const generateStepIds = (plan: typeof searchPlan) => {
        ...
        const searchSteps = plan.search_queries.flatMap((query, index) => {
          ...
          if (query.source === "ens") {
            return [{ id: `search-ens-${index}`, type: "ens", query }];
          }
          const searchType = query.source === "zerion" ? "zerion" : "web";
          return [
            {
