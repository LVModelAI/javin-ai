...
export const getEvmOnchainDataUsingBasescan = tool({
  description:
    "Get real-time data from Base-based blockchains using Basescan",
  parameters: z.object({
    userQuery: z.string().describe("Query of user."),
  }),
  execute: async ({ userQuery }: { userQuery?: string }) => {
    console.log("using getEvmOnchainDataUsingBasescan ...");
    try {
      console.log(
        "User query for getEvmOnchainDataUsingBasescan :",
        userQuery
      );
      const apiKey = process.env.BASESCAN_API_KEY;
      if (!apiKey) {
        throw new Error("Basescan API key not found");
      }

      const basescanOpenapidata = await loadOpenAPI(
        "https://raw.githubusercontent.com/PurrProof/etherscan-openapi/refs/heads/main/etherscan-openapi31-bundled.yml"
      );
      const basescanAllPathsAndDesc = await getAllPathsAndDesc(
        basescanOpenapidata
      );
      ...
      const aiAgentResponse = await generateText({
        ...
        prompt: JSON.stringify(
          `User query: "${userQuery}". Available API paths and descriptions: ${basescanAllPathsAndDesc}. Base URL: ${etherscanBaseURL}`
        ),
        tools: {
          ...
          getPathParametersAndBaseUrl: tool({
            ...
            execute: async ({ path }) => {
              ...
              const basescanPathsDetails = await getPathDetails(
                basescanAllPathsAndDesc,
                path
              );
              return basescanPathsDetails;
            },
          }),
          makeApiCall: tool({
            description: "Fetch real-time base chain data from etherscan API.",
            parameters: z.object({
            ...
            execute: async ({ url }) => {
              try {
                ...
                const fullUrl = `${url}&apikey=${apiKey}&chainid=8453`;
      ...
      console.error("Error in getEvmOnchainDataUsingBasescan:", error);
      Sentry.captureException(error);
      return {
        success: false,
